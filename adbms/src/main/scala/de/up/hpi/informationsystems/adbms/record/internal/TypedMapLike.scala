...
/** Template trait for immutable typed maps.
  *
...
trait TypedMapLike[K[+_ <: V], V, This <: TypedMapBase[K, V]] { self: This =>
  ...
  /** Adds a number of elements provided by a traversable object
    ...
    * @tparam T  the type-bound for values of the right traversable
    * @return a new `This` with the bindings of this map and those from `xs`.
    */
  def ++[T <: V](xs: TypedMapBase[K, T]): This =
    ctor(data.++(xs.iterator))

  /** Creates a new map from this one by removing all keys of another collection.
    *
  ...
  /** A new immutable map containing the updated map with a given key/value mapping.
    *
    * @param  key   the key
  ...
  /** Converts this map to a sequence.
    *
    * As with `toIterable`, it's lazy in this default implementation,
    * as this `TypedMapLike` may be lazy and unevaluated.
    *
